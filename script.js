...
/* ---------------------------
   ...
   (KEEP the values you already put here)
---------------------------- */
...
const TYPING_FRESH_MS = 3000; // consider typing active if updated within 3s

function setDebug(msg, show = true){
  console.log("CYOU:", msg);
  if (!debugStatus) return;
  debugStatus.textContent = msg;
  debugStatus.style.display = show ? "block" : "none";
  setTimeout(() => { if (debugStatus) debugStatus.style.display = "none"; }, 3500);
}
...
enterBtn.addEventListener("click", async () => {
  ...
  try {
    setDebug("Signing in anonymously...");
    await signInAnonymously(auth);
    setDebug("Sign-in requested.");
  } catch (err) {
...
onAuthStateChanged(auth, async (user) => {
  if (user) {
    ...
    setDebug("Signed in as " + displayName);
    enterUI();
    startPresence();
    subscribeToMessages();
    subscribeToPresence();
...
sendForm.addEventListener("submit", async (ev) => {
  ...
  try {
    ...
    setDebug("Failed to send message: " + (err.message || err));
  }
});
...
function subscribeToTyping(){
  ...
  onSnapshot(q, (snap) => {
    ...
    snap.forEach(d => {
      ...
      let recent = true;
      if (lastUpdated && lastUpdated.toMillis) {
    ...
    if (typers.length === 0) {
      ...
      typingIndicator.className = "typing-indicator typing-dots";
    } else {
      typingIndicator.textContent = `${typers.slice(0,3).join(", ")} are typing...`;
      typingIndicator.className = "typing-indicator typing-dots";
    }
  }, err => {
    console.error("subscribeToTyping error", err);
  });
}
...
async function startPresence(){
  ...
  try {
    ...
    console.error("startPresence initial set error", e);
  }
...
function subscribeToPresence(){
  ...
  onSnapshot(q, (snap) => {
    ...
    online.forEach(u => {
      ...
      li.innerHTML = `<span class="user-dot online" title="online"></span> <span class="online-name">${escapeHtml(u.name)}</span>`;
      onlineUsersEl.appendChild(li);
    });

    onlineCountEl.textContent = online.length;
  }, err => {
    console.error("subscribeToPresence error", err);
  });
}
...
function subscribeToMessages(){
  ...
  onSnapshot(q, (snap) => {
    ...
    setDebug("Messages subscription failed: " + (err.message || err));
  });
}
...
function renderMessage(id, data){
  ...
  time.className = "time";
  const ts = (data.createdAt && typeof data.createdAt.toDate === "function") ? data.createdAt.toDate() : new Date();
...
/* Mark last N visible messages as seen by current user */
