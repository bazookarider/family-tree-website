...
import {
  ...
  onSnapshot, query, orderBy, doc, setDoc, updateDoc, deleteDoc, getDoc, where, getDocs
...
/* ---------------------------
   ...
   - set typing doc with lastUpdated
   - subscribe shows only recent typers
---------------------------- */
let lastTypedAt = 0;
...
function subscribeToTyping(){
  ...
  onSnapshot(q, (snap) => {
    ...
    snap.forEach(d => {
      const data = d.data();
      // Only show those who have typing === true and were updated recently
      if (!data) return;
...
function renderMessage(id, data){
  ...
  time.className = "time";
  // createdAt may be serverTimestamp or missing; fallback to now
...
/* simple escape to avoid injecting raw HTML into user list */
