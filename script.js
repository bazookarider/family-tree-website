...
try {
    ...
    document.getElementById('system-status').style.background = 'red';
    document.getElementById('system-status').innerText = "Connection Error: " + e.message;
}

// --- GLOBAL FUNCTIONS (Must be outside any wrapper) ---
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById("loginBtn").onclick = async function() {
        ...
        catch (e) { alert("Error: " + e.message); this.innerText = "Log In"; }
    };
    ...
    document.getElementById("signupBtn").onclick = async function() {
        ...
        try {
            ...
            await db.ref(`users/${cred.user.uid}`).set({
                username: name, email, bio: "Hi!", category: "Member", isVerified: false, joined: firebase.database.ServerValue.TIMESTAMP, avatarStyle: "male"
            });
    ...
    document.getElementById("sendCommentBtn").onclick = async () => {
        ...
        if(txt && activePostIdForComments) {
    ...
    db.ref('posts').limitToLast(50).get().then(async (snap) => {
        ...
        posts.forEach(p => {
            if(p.uid === currentUser.uid || following[p.uid]) {
                ...
                const ava = getAvatar(p.username, "male"); // Feed default
