...
function startAviatorEngine() {
    setInterval(() => {
        ...
        if (loop < 4000) { // WAITING (4s)
            if(avState !== "WAITING") {
                avState = "WAITING";
                crashPoint = generateCrashPoint(); // USE NEW MATH
                ...
                document.getElementById('plane-icon').className = "ri-plane-fill";
                
                const btn = document.getElementById('bet-btn');
                if(avBet > 0 && !avCash) { 
                    avBet = 0; 
                    btn.innerText = "LOST"; btn.style.background = "#333";
                    updateReceipt("aviator", "LOST", 0);
                } else { 
            ...
            // Animation: Move Diagonally Up-Right
            const x = (flyTime / 8000) * 250; // Move Right
            const y = (flyTime / 8000) * -180; // Move Up
...
function doCashout() {
    ...
    updateReceipt("aviator", "WON", win);
}

// === SPIN GAME (FLIP ANIMATION) ===
function spinColor(choice) {
    ...
    const resultIcon = document.getElementById('spin-card-back');
    ...
    setTimeout(() => {
        ...
        resultIcon.innerText = outcome === 'lemon' ? "L" : "N"; // Just a letter for now
        ...
        if(choice === outcome) {
            ...
            alert("WIN! " + win);
            updateReceipt("spin", "WON", win);
        } else {
            alert("LOST!");
            updateReceipt("spin", "LOST", 0);
        }
...
function updateReceipt(game, status, amt) {
    const id = game + "-receipt";
    const el = document.getElementById(id);
    el.classList.remove('hidden');
    document.getElementById(game === 'aviator' ? 'av-rcpt-stat' : 'spin-rcpt-stat').innerText = status;
    document.getElementById(game === 'aviator' ? 'av-rcpt-amt' : 'spin-rcpt-amt').innerText = "₦" + amt;
}
...
function processWithdraw() {
    ...
    if(!bank || !acct) return alert("Fill Bank Details");
