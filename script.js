...
document.getElementById("resendVerify").onclick = () => currentUser && auth.currentUser.sendEmailVerification().then(() => alert("Sent!"));
...
function loadFeed() {
    ...
    db.ref('posts').limitToLast(50).get().then(async (snap) => {
        list.innerHTML = "";
        if (!snap.exists()) { list.innerHTML = "<div style='padding:20px;text-align:center;color:#999'>No posts.</div>"; return; }
...
function renderPost(p, container) {
    ...
    const avatar = getAvatar(p.username, "male"); // Default style for feed
        ...
        <div class="post-content">
            ...
            <div class="post-text">${p.text}</div>
             <div class="post-actions">
                <span class="like-btn ${isLiked ? 'liked' : ''}"><i class="${isLiked ? 'fa-solid' : 'fa-regular'} fa-heart"></i> ${likeCount || ''}</span>
                <span class="comment-btn"><i class="fa-regular fa-comment"></i> ${commentCount || ''}</span>
            </div>
        </div>`;
...
function loadComments(postId) {
    ...
    db.ref(`posts/${postId}`).get().then((postSnap) => {
        ...
        Object.entries(comments).forEach(([key, com]) => {
            const d = document.createElement("div"); d.className = "comment-item " + (com.pinned ? "comment-pinned" : "");
...
function renderUserList(users, targetId) {
    ...
    users.forEach(u => {
        ...
        div.innerHTML = `
            ...
            <div class="user-info"><div class="user-name-list">${u.username} ${u.isVerified ? '<i class="fa-solid fa-circle-check verified-badge"></i>' : ''}</div><div class="user-bio-list">${u.category || 'Member'}</div></div>
...
async function checkFollowStatus(targetUid) {
    ...
    if (amIFollowing && isHeFollowing) {
        msgBtn.disabled = false; msgBtn.textContent = "Chat Now";
        ...
        msgBtn.disabled = true; msgBtn.textContent = "Locked 🔒";
...
function startChat(uid, name) {
    ...
    document.getElementById("userProfileModal").classList.add("hidden"); document.getElementById("chat-room").classList.remove("hidden");
